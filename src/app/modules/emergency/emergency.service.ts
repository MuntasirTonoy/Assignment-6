import { AmbulanceStatus, PriorityLevel, type Prisma, RequestStatus } from '@prisma/client';
...
const createEmergency = async (patientId: string, payload: ICreateEmergencyPayload) => {
...
const getEmergencyById = async (id: string, userId: string, role: string) => {
  ...
  if (role === 'PATIENT' && emergency.patientId !== userId) {
    throw new AppError(403, 'Forbidden: You cannot access another patient emergency record');
  }
...
const dispatchEmergency = async (
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (emergency.status !== RequestStatus.PENDING) {
      throw new AppError(400, `Cannot dispatch an emergency that is already ${emergency.status}`);
    }
    ...
    if (ambulance.status !== AmbulanceStatus.AVAILABLE) {
      throw new AppError(409, `Ambulance is currently ${ambulance.status} and cannot be dispatched`);
    }
...
const cancelEmergency = async (id: string, userId: string, role: string) => {
  return await prisma.$transaction(async (tx) => {
    ...
    if (role === 'PATIENT' && emergency.patientId !== userId) {
      throw new AppError(403, 'Forbidden: You cannot cancel another patient request');
    }
    ...
    if (
      ...
      throw new AppError(400, `Cannot cancel an emergency request that is ${emergency.status}`);
    }
